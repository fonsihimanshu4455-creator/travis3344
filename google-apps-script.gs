...
const ADMIN_EMAIL = "a09098375@gmail.com";
...
// JSONP support: if ?callback=fnName is passed, wraps JSON in fnName(...)
function doGet(e) {
  const callback = sanitize_(e && e.parameter && e.parameter.callback);
  const result = handleGetRequest_(e);
...
function handleGetRequest_(e) {
  ...
  if (action === "login") {
    ...
    return { ok: true, token: ADMIN_SESSION_TOKEN, admin: ADMIN_LOGIN_USERNAME };
  }
  ...
  if (action === "config") {
    ...
    return { ok: true, websites: config.websites, whatsappHelpline: config.whatsappHelpline };
  }
  ...
  if (action === "save_config") {
    ...
    if (!websites.length) {
      return { ok: false, error: "At least one website is required" };
    }

    writeConfig_(websites, nextHelpline);

    return { ok: true, message: "Config saved", websites: websites, whatsappHelpline: nextHelpline };
  }

  return { ok: true, message: "Signup endpoint is running." };
}
...
function readConfig_() {
  ...
  if (!websites.length) websites = ["bpexch"];

  let whatsappHelpline = normalizePhone_(map.whatsapp_helpline);
  if (!whatsappHelpline) whatsappHelpline = "919999999999";

  if (!map.websites_json || !map.whatsapp_helpline) {
    writeConfig_(websites, whatsappHelpline);
  }

  return { websites: websites, whatsappHelpline: whatsappHelpline };
}
...
function parseWebsites_(value) {
  if (Array.isArray(value)) return sanitizeWebsites_(value);
  const raw = sanitize_(value);
  if (!raw) return [];
  let parsed = [];
  try { parsed = JSON.parse(raw); } catch (e) { parsed = raw.split(","); }
